...
def test_create_workflow_uses_local_when_requested() -> None:
...
def test_create_workflow_auto_prefers_langgraph_when_available() -> None:
    pytest.importorskip("langgraph")

    workflow = create_workflow(build_retriever())

    assert isinstance(workflow, LangGraphRAGWorkflow)
